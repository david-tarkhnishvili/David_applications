...
def _render_header() -> None:
    ...
    st.markdown(
        f"""
        <style>
            :root {{ --leslie-banner-height: 300px; }}
            [data-testid="stHeader"], [data-testid="stToolbar"] {{ display: none !important; height: 0 !important; }}
            [data-testid="stAppViewContainer"] .main .block-container, .block-container {{ padding-top: var(--leslie-banner-height) !important; padding-left: 0 !important; padding-right: 0 !important; max-width: 100% !important; }}
            [data-testid="stSidebar"] > div:first-child {{ padding-top: var(--leslie-banner-height) !important; }}
            .block-container > div:not(:first-child) {{ padding-left: 1rem; padding-right: 1rem; }}
            .leslie-hero {{ position: fixed; top: 0; left: 0; right: 0; z-index: 2147483000; pointer-events: none; height: var(--leslie-banner-height); width: 100vw; margin: 0; padding: 1.65rem max(1.25rem, calc((100vw - 1260px) / 2)); box-sizing: border-box; background: linear-gradient(112deg, #0e3735 0%, #17605b 48%, #d7a441 100%); color: #f7fbf7; display: grid; grid-template-columns: minmax(320px, 0.95fr) minmax(560px, 1.05fr); gap: 1.5rem; align-items: center; box-shadow: 0 8px 24px rgba(17, 63, 60, 0.18); }}
            .leslie-title {{ font-size: clamp(2rem, 2.75vw, 3rem); font-weight: 780; line-height: 1.05; margin: 0 0 0.7rem 0; letter-spacing: 0; }}
            .leslie-subtitle {{ font-size: 1.08rem; line-height: 1.42; max-width: 54rem; margin: 0; color: rgba(247, 251, 247, 0.92); }}
            .leslie-visual {{ display: grid; grid-template-columns: 300px minmax(360px, 1fr); gap: 1rem; align-items: stretch; }}
            .silhouette-strip {{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.65rem; }}
            .silhouette-tile {{ height: 94px; border-radius: 8px; background: rgba(226, 238, 226, 0.42); border: 1px solid rgba(255, 255, 255, 0.38); display: flex; align-items: center; justify-content: center; overflow: hidden; }}
            ...
            .population-card {{ min-width: 0; border-radius: 8px; background: rgba(226, 238, 226, 0.22); border: 1px solid rgba(255, 255, 255, 0.34); padding: 0.95rem 1rem 0.72rem 1rem; }}
            .population-card img {{ width: 100%; height: 164px; display: block; object-fit: contain; }}
            .mini-label {{ font-size: 0.78rem; color: rgba(247, 251, 247, 0.84); margin-top: 0.2rem; }}
            ...
            @media (max-width: 760px) {{ :root {{ --leslie-banner-height: 210px; }} .leslie-hero {{ grid-template-columns: minmax(0, 0.82fr) minmax(0, 1.18fr); padding: 0.75rem 0.65rem; gap: 0.55rem; align-items: center; }} .leslie-title {{ font-size: 1.22rem; line-height: 1.08; margin-bottom: 0; word-break: normal; overflow-wrap: normal; }} .leslie-subtitle {{ display: none; }} .leslie-visual {{ grid-template-columns: 1fr; gap: 0.38rem; }} .silhouette-strip {{ grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.32rem; }} .silhouette-tile {{ height: 42px; border-radius: 6px; }} .population-card {{ padding: 0.34rem 0.42rem 0.25rem 0.42rem; border-radius: 6px; }} .population-card img {{ height: 58px; }} .mini-label {{ font-size: 0.52rem; line-height: 1.05; }} }}
        </style>
