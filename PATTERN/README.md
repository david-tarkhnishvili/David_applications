...
- launch matcher jobs
- launch pattern-variation jobs
- let you edit many parameters from the browser
- show recent jobs
- show approximate progress
- show summaries, result tables, previews, and logs
...
The app also includes a dedicated `Pattern Variation` section for:

- building a consensus dorsal-pattern image
- measuring per-image and per-specimen deviation from that consensus
- measuring left-right dorsal-pattern asymmetry

This analysis uses the same `5-point curved belt` landmarking as the current best matcher.
