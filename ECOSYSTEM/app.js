...
const els = {
  ...
  showPasteImport: document.querySelector("#showPasteImport"),
  pasteImportPanel: document.querySelector("#pasteImportPanel"),
  pasteImportText: document.querySelector("#pasteImportText"),
  importPastedSpecies: document.querySelector("#importPastedSpecies"),
  clearPastedSpecies: document.querySelector("#clearPastedSpecies"),
...
els.file.addEventListener("change", async event => {
  ...
  if (!file) return;
  loadSpeciesText(await file.text(), file.name);
  event.target.value = "";
});

els.showPasteImport.addEventListener("click", () => {
  els.pasteImportPanel.hidden = !els.pasteImportPanel.hidden;
  if (!els.pasteImportPanel.hidden) els.pasteImportText.focus();
});

els.importPastedSpecies.addEventListener("click", () => {
  loadSpeciesText(els.pasteImportText.value, "pasted list");
});

els.clearPastedSpecies.addEventListener("click", () => {
  els.pasteImportText.value = "";
  els.pasteImportText.focus();
});

els.loadSample.addEventListener("click", async () => {
  setStatus("busy", "Loading species1.csv...");
  try {
    const response = await fetch("species1.csv", { cache: "no-store" });
    if (!response.ok) throw new Error("Could not fetch species1.csv");
    loadSpeciesText(await response.text(), "species1.csv");
  } catch (error) {
    setStatus("", "Could not load species1.csv. Upload a CSV or paste a list instead.");
  }
});
...
function parseCsv(text) {
  ...
  const speciesIndex = headers.findIndex(h => ["species", "taxon", "name", "scientific_name", "scientific name", "species name", "latin name"].includes(h));
  if (speciesIndex < 0) throw new Error("CSV needs a species column, or paste one species name per line.");
  return lines.map(line => splitCsvLine(line)[speciesIndex]?.trim()).filter(Boolean);
}

function parseSpeciesText(text) {
  const cleanText = text.trim();
  if (!cleanText) throw new Error("Paste or upload a species list first.");
  const lines = cleanText.replace(/^\uFEFF/, "").split(/\r?\n/).map(line => line.trim()).filter(Boolean);
  const firstLine = lines[0] || "";
  const firstCells = splitCsvLine(firstLine).map(cell => cell.trim().toLowerCase());
  const hasCsvHeader = firstCells.some(cell => ["species", "taxon", "name", "scientific_name", "scientific name", "species name", "latin name"].includes(cell));
  if (hasCsvHeader) return parseCsv(cleanText);
  if (lines.some(line => line.includes(","))) {
    return lines.map(line => splitCsvLine(line)[0]?.trim()).filter(Boolean);
  }
  return lines;
}
...
function loadSpeciesText(text, sourceName) {
  try {
    species = parseSpeciesText(text).map((name, index) => applySpeciesOverride(classifySpecies(name, index)));
...
function renderNetwork() {
  if (!species.length) {
    els.network.innerHTML = "<p class='empty-state'>Upload a CSV, paste a list, or load species1.csv to draw the interaction web.</p>";
    return;
  }
...
function renderMatrix() {
  if (!species.length) {
    els.simpleMatrix.innerHTML = "<tr><td class='empty-state'>Upload a CSV, paste a list, or load species1.csv to build the matrix.</td></tr>";
...
function renderSources() {
  if (!species.length) {
    els.sources.innerHTML = "<article class='source-card'><h3>No species loaded</h3><p>Upload a CSV, paste a list, or load species1.csv, then press Consult internet. Progress will appear in the status bar above.</p></article>";
    return;
  }
