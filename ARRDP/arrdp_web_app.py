...
DEFAULT_PROJECT_DIR = os.environ.get("ARRDP_PROJECT_DIR") or (
    str(PORTABLE_PROJECT_DIR) if PORTABLE_PROJECT_DIR else str(SCRIPT_DIR.parent)
)
DEFAULT_HOST = os.environ.get("ARRDP_HOST", "127.0.0.1")
DEFAULT_PORT = env_int("ARRDP_PORT", env_int("PORT", 8094))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Local web app for ARRDP workflows.")
    parser.add_argument("--project-dir", default=DEFAULT_PROJECT_DIR, help="Project folder containing gallery/query/config/outputs.")
...
def render_home_page(message: str | None = None, *, error: bool = False) -> bytes:
    ...
    body = f"""
<section class="panel">
  <h1>ARRDP - Amphibian and Reptile Recognition by Dorsal Pattern</h1>
...
"""
    return html_page("ARRDP - Dorsal Pattern Recognition", body)
...
def is_allowed_path(path: Path) -> bool:
    ...
    try:
        ...
        return False
    return any(resolved == root or is_relative_to(resolved, root) for root in allowed_roots)
...
class DarevskiaHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if parsed.path == "/manual":
            self.respond(HTTPStatus.OK, render_markdown_as_pre(MANUAL_PATH, "ARRDP User Manual"))
            return
        if parsed.path == "/spec":
...
def main() -> None:
    ...
    print(f"ARRDP web app running at {url}")
    print(f"Project folder: {DEFAULT_PROJECT_DIR}")
    print("Authentication is disabled. This portable edition is for local use only.")
